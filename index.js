...
// PATCH: Update event to mark it inactive
app.patch("/api/events/:id", async (req, res) => {
  ...
  try {
    const result = await db_events.markEventInactive(id);
    if (result.success) {
      res
        .status(200)
        .json({ message: "Event successfully marked as inactive." });
    } else {
      res.status(400).json({ error: "Failed to mark event as inactive." });
    }
  } catch (error) {
    console.error("Error marking event inactive:", error);
    res.status(500).json({ error: "Internal server error." });
  }
});

app.patch("/api/restore/:id", async (req, res) => {
  const { id } = req.params;
  const { is_active, deleted_at } = req.body;
  ...
  try {
    const result = await db_events.restoreEvent(id, is_active, deleted_at);
    if (result.success) {
      res
        .status(200)
        .json({ message: "Event successfully marked as inactive." });
